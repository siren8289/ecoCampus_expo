...
이 모듈은 데이터베이스 초기화 시 사용할 테스트/개발용 데이터를 정의합니다.
seed_data() 함수를 호출하면 모든 테이블에 초기 데이터가 삽입됩니다.
"""
from models import (
    db,
    User,
...
def seed_data():
    """
    초기 목업 데이터를 데이터베이스에 삽입하는 함수
    
    이 함수는 데이터베이스가 비어있을 때만 실행됩니다.
    이미 데이터가 있으면 실행하지 않아 중복 삽입을 방지합니다.
    ...
    삽입되는 데이터:
        ...
        등
    """
    # 이미 데이터가 있는지 확인 (사용자 테이블 기준)
    if User.query.first():
        # 이미 시드된 것으로 간주하고 함수 종료
        return
    ...
    db.session.add(user)
    # flush()를 호출하여 user.id를 즉시 생성 (다른 테이블에서 참조하기 위해)
    db.session.flush()
